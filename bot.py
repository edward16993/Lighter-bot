...
def record_close(exit_px,reason,side):
    global stats
    if side=="LONG": pnl=round((exit_px-stats["entry_price"])*stats["entry_size"],4)
    else: pnl=round((stats["entry_price"]-exit_px)*stats["entry_size"],4)
    ...
    stats["history"].append({"no":stats["total_trades"],"side":side,
        "entry":stats["entry_price"],"exit":exit_px,"pnl":pnl,
        "reason":reason,"new_margin":new_m,
        "time":datetime.now().strftime("%Y-%m-%d %H:%M")})
    stats["history"]=stats["history"][-20:]
    save_stats()
    return pnl,new_m,"WIN" if pnl>=0 else "LOSS"
def p(v): return "*"+str(v)+"*"
...
async def strategy_loop():
    ...
    try:
        ...
        for pos2 in (getattr(r,"positions",[]) or []):
            if getattr(pos2,"market_index",-1)==MKT_IDX:
                s=float(getattr(pos2,"base_amount",0) or 0)
                if s!=0:
                    cur_pos="LONG" if s>0 else "SHORT"
                    entry_px=float(stats.get("entry_price",0))
                    entry_sz=abs(s); entry_mg=float(stats.get("entry_margin",0))
                    sl_px=float(stats.get("sl_price",0)); tp_px=float(stats.get("tp_price",0))
    except Exception as e: logger.error("Restore error: %s",e)
    ...
    while True:
        try:
            df=await fetch_candles(300); df=calc_indicators(df); df=df.dropna()
            ...
            if cur_pos is None:
                if long_sig and stats["current_margin"]>=MIN_MAR:
                    mg=stats["current_margin"]; si=calc_size(mg,price)
                ...
                unr=round((price-entry_px)*entry_sz,4); reason=None
                ...
                unr=round((entry_px-price)*entry_sz,4); reason=None
                if price<=tp_px: reason="TP"
                elif price>=sl_px: reason="SL"
            ...
            logger.error("Loop error: %s",e); await send_tg("Loop error: "+str(e))
...
async def cmd_status(u,c):
    try:
        df=await fetch_candles(300); df=calc_indicators(df); df=df.dropna(); curr=df.iloc[-1]
...
async def cmd_stats(u,c):
    t=stats["total_trades"]; wr=round(stats["wins"]/max(t,1)*100,1)
    g=round((stats["current_margin"]-START_M)/START_M*100,1)
    lt=stats.get("long_trades",0); sh=stats.get("short_trades",0)
    sg="+" if g>=0 else ""
    await u.message.reply_text(
        "*Stats*\n"
...
async def cmd_history(u,c):
    h=stats.get("history",[])
    if not h: await u.message.reply_text("No trades yet!"); return
    lines=["*Last trades:*"]
    for t in h[-5:]:
        sg="+" if t["pnl"]>=0 else ""
