...
MARKETS={
    "ETH":{"symbol":"ETH-USDT-SWAP","market_id":0,"stats_file":"eth_stats.json",
           ...
           "decimals":3,"min_size":0.002,"price_dec":2,"size_mult":10000,"emoji":"ETH"},
    "BNB":{"symbol":"BNB-USDT-SWAP","market_id":25,"stats_file":"bnb_stats.json",
           "start_margin":float(os.environ.get("BNB_MARGIN","10")),
           "decimals":2,"min_size":0.02,"price_dec":4,"size_mult":100,"emoji":"BNB"}
}
...
async def place_order(coin,side,size,price,reduce_only=False):
    mkt=MARKETS[coin]
    slip=0.002
    pdec=mkt.get("price_dec",2)
    if side=="BUY": op=float(price)*(1+slip)
    else: op=float(price)*(1-slip)
    ba=int(round(float(size)*mkt.get("size_mult",10000)))
    op_int=int(round(op*(10**pdec)))
...
async def send_tg(msg):
    try: await tg_app.bot.send_message(chat_id=TG_CHAT,text=msg)
...
async def strategy_loop(coin):
    ...
    await send_tg(
        "*"+em+" "+coin+" Bot Started*\n"
        "Margin: $*"+str(s["stats"]["current_margin"])+"* | "+ps+"\n"
    ...
    while True:
        try:
            ...
            bear_trend=price<float(curr["ema200"])
            trending=adx>ADX_MIN
            atr_ratio=float(curr["atr_ratio"]) if not pd.isna(curr["atr_ratio"]) else 1.0
            good_vol=atr_ratio>0.7
            long_sig=bull_cross and bull_trend and trending and good_vol
            if coin=="ETH":
                short_sig=bear_cross and (rsi<50) and trending and bear_trend and good_vol
            else:
                short_sig=bear_cross and (rsi<50) and trending and good_vol
            logger.info("%s=$%.2f RSI=%.1f ADX=%.1f pos=%s",coin,price,rsi,adx,s["position"])

            # BNB: Check if paused due to 3 consecutive SL
            if coin=="BNB" and s["pause_until"] is not None:
                if datetime.now() < s["pause_until"]:
                    remaining=int((s["pause_until"]-datetime.now()).total_seconds()/60)
                    logger.info("BNB paused for %d more minutes",remaining)
                    await asyncio.sleep(INTERVAL)
                    continue
                else:
                    s["pause_until"]=None
                    s["consec_sl"]=0
                    await send_tg("BNB *Resume!* Pause ended - Trading again! 🟢")
            ...
            if s["position"] is None:
                ...
                if reason:
                    ...
                    try:
                        ...
                        s["position"]=None
                        # ATH notification
                        if new_m > s["stats"].get("peak_margin", s["stats"]["current_margin"]):
                            s["stats"]["peak_margin"] = new_m
                            await send_tg("ATH! "+em+" "+coin+" new high: $"+str(new_m)+" 🎉")
                        # Win/Loss streak
                        h=s["stats"].get("history",[])
                        if len(h)>=3:
                            last3=[t["pnl"]>=0 for t in h[-3:]]
                            if all(last3): await send_tg("WIN STREAK! "+em+" "+coin+" 3 wins in a row! 🔥")
                            elif not any(last3): await send_tg("LOSS STREAK! "+em+" "+coin+" 3 losses in a row! ⚠️")
                        # BNB: Track consecutive SL hits
                        if coin=="BNB":
                            if reason=="SL":
                                s["consec_sl"]=s.get("consec_sl",0)+1
                                if s["consec_sl"]>=3:
                                    s["pause_until"]=datetime.now()+__import__("datetime").timedelta(hours=1)
                                    s["consec_sl"]=0
                                    await send_tg("BNB ⚠️ *3 SL in a row!*\nPausing 1 hour - Market too choppy!\nResume: "+s["pause_until"].strftime("%H:%M"))
                            else:
                                s["consec_sl"]=0
                    except Exception as e:
...
async def cmd_start(u,c):
    ...
    msg+="\n/status /signal /stats /history /balance /pnl /week"
...
async def cmd_balance(u,c):
    try:
        ...
        acc=lighter.AccountApi(api)
        r=await acc.account(by="index",value=str(ACC_IDX)); await api.close()
        r_dict=r.to_dict() if hasattr(r,"to_dict") else r.__dict__
        accounts=r_dict.get("accounts",[])
        col=None; upnl=None
        if accounts and len(accounts)>0:
            acct=accounts[0]
            if hasattr(acct,"to_dict"): acct=acct.to_dict()
            elif hasattr(acct,"__dict__"): acct=acct.__dict__
            for k,v in acct.items():
                kl=k.lower()
                if col is None and ("collateral" in kl or "balance" in kl or "equity" in kl) and v is not None:
                    col=v
                if upnl is None and ("pnl" in kl or "unrealized" in kl) and v is not None:
                    upnl=v
        col_str=str(round(float(col),2)) if col is not None else "N/A"
        upnl_str=str(round(float(upnl),2)) if upnl is not None else "0.00"
        lines=["Balance"]
        lines.append("Collateral: $"+col_str)
        lines.append("Unrealized: $"+upnl_str)
        lines.append("")
        for coin in MARKETS:
            s=state[coin]["stats"]; em=MARKETS[coin]["emoji"]
            lines.append(em+" "+coin+" Margin: $"+str(s["current_margin"]))
...
def make_report(label):
    ...
    for coin in MARKETS:
        ...
        mg=s["current_margin"]
        g=round((mg-sm)/sm*100,1)
        sg="+" if pnl>=0 else ""
        gg="+" if g>=0 else ""
        total_pnl+=pnl
        lines.append(em+" "+coin)
        lines.append("Trades:"+str(s["total_trades"])+" W:"+str(s["wins"])+" L:"+str(s["losses"]))
        lines.append("PnL: "+sg+"$"+str(pnl))
        lines.append("Margin: $"+str(mg)+" ("+gg+str(g)+"%)")
        lines.append("")
    lines.append("---")
    sg="+" if total_pnl>=0 else ""
    lines.append("Total: "+sg+"$"+str(round(total_pnl,4)))
    return "\n".join(lines)
...
async def daily_report():
    ...
    while True:
        try:
            ...
            logger.info("Next report in %.0f sec",wait)
            await asyncio.sleep(wait)
            now2=datetime.now()
            # Daily report
            await send_tg(make_report("Daily Report - "+now2.strftime("%d %b")))
            # Weekly = Sunday (weekday=6)
            if now2.weekday()==6:
                await asyncio.sleep(2)
                await send_tg(make_report("Weekly Report - "+now2.strftime("%d %b")))
            # Monthly = 1st of month
            if now2.day==1:
                await asyncio.sleep(2)
                await send_tg(make_report("Monthly Report - "+now2.strftime("%b %Y")))
        except Exception as e:
...
async def cmd_pnl(u,c):
    ...
    lines=["*Today PnL - "+datetime.now().strftime("%d %b")+"*","---"]
    total=0.0
    for coin in MARKETS:
        em=MARKETS[coin]["emoji"]
        h=state[coin]["stats"].get("history",[])
        td=[t for t in h if t.get("time","").startswith(today)]
        pnl=round(sum(t["pnl"] for t in td),4)
        total+=pnl
        sg="+" if pnl>=0 else ""
        icon="✅" if pnl>=0 else "❌"
        lines.append(em+" *"+coin+"*: "+icon+" *"+sg+"$"+str(pnl)+"* ("+str(len(td))+" trades)")
    lines.append("---")
    sg="+" if total>=0 else ""
    lines.append("Total: *"+sg+"$"+str(round(total,4))+"*")
    await u.message.reply_text("\n".join(lines),parse_mode="Markdown")

async def cmd_week(u,c):
    from datetime import timedelta
    week_ago=(datetime.now()-timedelta(days=7)).strftime("%Y-%m-%d")
    d1=(datetime.now()-timedelta(days=7)).strftime("%d %b")
    d2=datetime.now().strftime("%d %b")
    lines=["*Weekly Summary ("+d1+" - "+d2+")*","---"]
    total_pnl=0.0
    for coin in MARKETS:
        em=MARKETS[coin]["emoji"]
        s=state[coin]["stats"]
        sm=MARKETS[coin]["start_margin"]
        h=s.get("history",[])
        wt=[t for t in h if t.get("time","")>=week_ago]
        wpnl=round(sum(t["pnl"] for t in wt),4)
        ww=sum(1 for t in wt if t["pnl"]>=0)
        wwr=round(ww/max(len(wt),1)*100,1)
        total_pnl+=wpnl
        sg="+" if wpnl>=0 else ""
        g=round((s["current_margin"]-sm)/sm*100,1)
        gg="+" if g>=0 else ""
        lines.append(em+" *"+coin+"*")
        lines.append("Trades: *"+str(len(wt))+"* | WR: *"+str(wwr)+"%*")
        lines.append("PnL: *"+sg+"$"+str(wpnl)+"*")
        lines.append("Margin: *$"+str(s["current_margin"])+"* ("+gg+str(g)+"%)")
        lines.append("")
    lines.append("---")
    sg="+" if total_pnl>=0 else ""
    lines.append("Total: *"+sg+"$"+str(round(total_pnl,4))+"*")
