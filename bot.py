...
async def place_order(coin,side,size,price,reduce_only=False):
    ...
    tx,txh,err=await signer.create_order(
        ...
        order_expiry=int(datetime.now().timestamp())+60)
            ...
            if s["position"] is None:
                if long_sig and s["stats"]["current_margin"]>=MIN_MAR:
                    ...
                    try:
                        ...
                        save_stats(coin)
                        await send_tg(
                            "OK "+em+" "+coin+" LONG Opened!\n"
                            "Entry: $"+str(round(price,2))+" | Size: "+str(si)+" "+coin+"\n"
                            "SL: $"+str(nsl)+" | TP: $"+str(ntp)+"\n"
                            "Margin: $"+str(mg))
                    except Exception as e:
                        s["position"]=None
                        await send_tg("WARN "+em+" "+coin+" LONG Failed: "+str(e))

                elif short_sig and s["stats"]["current_margin"]>=MIN_MAR:
                    mg=s["stats"]["current_margin"]; si=calc_size(coin,mg,price)
                    nsl=round(price+(SL_MULT*atr),2); ntp=round(price-(TP_MULT*atr),2)
                    await send_tg(
                        em+" "+coin+" SHORT Signal! DOWN\n"
                        "Price: $"+str(round(price,2))+" | ADX: "+str(round(adx,1))+"\n"
                        "SL: $"+str(nsl)+" | TP: $"+str(ntp)+"\n"
                        "Margin: $"+str(mg)+" x "+str(LEV)+"x")
                    try:
                        ...
                        save_stats(coin)
                        await send_tg(
                            "OK "+em+" "+coin+" SHORT Opened!\n"
                            "Entry: $"+str(round(price,2))+" | Size: "+str(si)+" "+coin+"\n"
                            "SL: $"+str(nsl)+" | TP: $"+str(ntp)+"\n"
                            "Margin: $"+str(mg))
                    except Exception as e:
                        s["position"]=None
                        await send_tg("WARN "+em+" "+coin+" SHORT Failed: "+str(e))
                ...
                if reason:
                    ...
                    try:
                        ...
                        await send_tg("WARN "+em+" "+coin+" LONG Close Failed: "+str(e))
                ...
                if reason:
                    ...
                    try:
                        ...
                        await send_tg("WARN "+em+" "+coin+" SHORT Close Failed: "+str(e))

        except Exception as e:
            logger.error("%s loop error: %s",coin,e)
            await send_tg("WARN "+em+" "+coin+" Error: "+str(e))
    ...
    for coin in MARKETS:
        ...
        try:
            ...
            if bull_cross and bull_trend and trending:
                sig="LONG NOW! SL: $"+str(round(price-SL_MULT*atr,2))+" TP: $"+str(round(price+TP_MULT*atr,2))
            elif bear_cross and rsi<50 and trending:
                sig="SHORT NOW! SL: $"+str(round(price+SL_MULT*atr,2))+" TP: $"+str(round(price-TP_MULT*atr,2))
...
async def main():
    ...
    await send_tg(
        "Bot Started!\n"
        "ETH $"+str(state["ETH"]["stats"]["current_margin"])
        +" | BNB $"+str(state["BNB"]["stats"]["current_margin"])+"\n"
        +str(LEV)+"x | ALMA+EMA200+ADX | 5min OK")
