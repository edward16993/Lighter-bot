...
async def place_order(coin,side,size,price,reduce_only=False):
    ...
    size_decimals=mkt["decimals"]
    price_decimals=mkt.get("price_decimals",2)
    ba=int(round(float(size)*(10**size_decimals)))
    op_int=int(round(float(op)*(10**price_decimals)))
    tx,txh,err=await signer.create_order(
        market_index=int(mkt["market_id"]),
        client_order_index=int(datetime.now().timestamp()*1000),
        base_amount=ba,
        price=op_int,
        ...
        order_expiry=int(datetime.now().timestamp()*1000)+60000)
...
async def send_tg(msg):
    try: await tg_app.bot.send_message(chat_id=TG_CHAT,text=msg,parse_mode="HTML")
...
async def strategy_loop(coin):
    ...
    try:
        ...
        r=await acc.account(by="index",value=str(ACC_IDX))
        await api.close()
        ...
        logger.error("%s restore: %s",coin,e)
        ...
        try:
            ...
            if s["position"] is None:
                if long_sig and s["stats"]["current_margin"]>=MIN_MAR:
                    ...
                    await send_tg(
                        em+" <b>"+coin+" LONG Signal! \U0001f4c8</b>\n"
                        "Price: <b>$"+str(round(price,2))+"</b> | ADX: <b>"+str(round(adx,1))+"</b>\n"
                        "\U0001f6e1 SL: <b>$"+str(nsl)+"</b> | \U0001f3af TP: <b>$"+str(ntp)+"</b>\n"
                        "\U0001f4b0 Margin: <b>$"+str(mg)+"</b> x "+str(LEV)+"x")
                    try:
                        ...
                        save_stats(coin)
                        await send_tg(
                            "\u2705 "+em+" <b>"+coin+" LONG Opened!</b>\n"
                            "\U0001f4cd Entry: <b>$"+str(round(price,2))+"</b> | Size: <b>"+str(si)+" "+coin+"</b>\n"
                            "\U0001f6e1 SL: <b>$"+str(nsl)+"</b> | \U0001f3af TP: <b>$"+str(ntp)+"</b>\n"
                            "\U0001f4b0 Margin: <b>$"+str(mg)+"</b>")
                    except Exception as e:
                        s["position"]=None
                    ...
                    nsl=round(price+(SL_MULT*atr),2); ntp=round(price-(TP_MULT*atr),2)
                    await send_tg(
                        em+" <b>"+coin+" SHORT Signal! \U0001f4c9</b>\n"
                        "Price: <b>$"+str(round(price,2))+"</b> | ADX: <b>"+str(round(adx,1))+"</b>\n"
                        "\U0001f6e1 SL: <b>$"+str(nsl)+"</b> | \U0001f3af TP: <b>$"+str(ntp)+"</b>\n"
                        "\U0001f4b0 Margin: <b>$"+str(mg)+"</b> x "+str(LEV)+"x")
                    try:
                ...
                if reason:
                    icon="\U0001f3af" if reason=="TP" else "\U0001f6d1" if reason=="SL" else "\U0001f504"
                    sg="+" if unr>=0 else ""
                    await send_tg(
                        icon+" "+em+" <b>"+coin+" LONG "+reason+"!</b>\n"
                        "Price: <b>$"+str(round(price,2))+"</b>\n"
                        "Unrealized: <b>"+sg+"$"+str(unr)+"</b> ("+sg+str(unr_pct)+"%)")
                    try:
                        ...
                        sg="+" if pnl>=0 else ""
                        await send_tg(
                            outcome+" "+em+" <b>"+coin+" LONG #"+str(s["stats"]["total_trades"])+"</b> ["+reason+"]\n"
                            "\U0001f4cd Entry: <b>$"+str(s["entry_px"])+"</b> Exit: <b>$"+str(round(price,2))+"</b>\n"
                            "\U0001f4b5 PnL: <b>"+sg+"$"+str(pnl)+"</b> ("+sg+str(pnl_pct)+"%)\n"
                            "\U0001f4ca WR: <b>"+str(wr)+"%</b> | \U0001f4b0 Margin: <b>$"+str(new_m)+"</b>")
                        s["position"]=None
                    except Exception as e:
                ...
                if reason:
                    ...
                    try:
                        ...
                        sg="+" if pnl>=0 else ""
                        await send_tg(
                            outcome+" "+em+" <b>"+coin+" SHORT #"+str(s["stats"]["total_trades"])+"</b> ["+reason+"]\n"
                            "\U0001f4cd Entry: <b>$"+str(s["entry_px"])+"</b> Exit: <b>$"+str(round(price,2))+"</b>\n"
                            "\U0001f4b5 PnL: <b>"+sg+"$"+str(pnl)+"</b> ("+sg+str(pnl_pct)+"%)\n"
                            "\U0001f4ca WR: <b>"+str(wr)+"%</b> | \U0001f4b0 Margin: <b>$"+str(new_m)+"</b>")
                        s["position"]=None
...
async def cmd_status(u,c):
    ...
    for coin in MARKETS:
        ...
        try:
            ...
            trend="\U0001f7e2 Bull" if price>ema200 else "\U0001f534 Bear"
            adx_s="\U0001f4c8 Trending" if adx>ADX_MIN else "\U0001f634 Sideways"
            ps=s["position"] if s["position"] else "Wait"
            extra=""
            if s["position"] and s["entry_px"]>0:
                ...
                sg="+" if unr>=0 else ""
                extra=("\nUnrealized: <b>"+sg+"$"+str(unr)+"</b> ("+sg+str(unr_pct)+"%)"
                    "\nSL: <b>$"+str(s["sl_px"])+"</b> | TP: <b>$"+str(s["tp_px"])+"</b>")
            msg+=(em+" <b>"+coin+"</b> | <b>$"+str(round(price,2))+"</b> | "+trend+"\n"
                "RSI: <b>"+str(rsi)+"</b> | ATR: $"+str(atr)+"\n"
                "ADX: <b>"+str(adx)+"</b> | "+adx_s+"\n"
                "EMA200: $"+str(ema200)+"\n"
                "Position: <b>"+ps+"</b>"+extra+"\n"
                "\U0001f4b0 Margin: <b>$"+str(s["stats"]["current_margin"])+"</b>\n\n")
        except Exception as e:
            msg+=em+" <b>"+coin+"</b> ERROR: "+str(e)+"\n\n"
    await u.message.reply_text(msg,parse_mode="HTML")
    ...
    for coin in MARKETS:
        ...
        try:
            ...
            if bull_cross and bull_trend and trending:
                ...
                sig="\U0001f7e1 Bullish - Wait cross"
            elif rsi<50 and curr["fast"]<curr["slow"]:
                sig="\U0001f7e1 Bearish - Wait cross"
            else:
                sig="\u26aa No signal"
            msg+=(em+" <b>"+coin+"</b> | <b>$"+str(round(price,2))+"</b>\n"
                "RSI: <b>"+str(rsi)+"</b> | ADX: <b>"+str(adx)+"</b> "+adx_s+"\n"
                +sig+"\n\n")
        except Exception as e:
            msg+=em+" <b>"+coin+"</b> ERROR: "+str(e)+"\n\n"
    await u.message.reply_text(msg,parse_mode="HTML")
...
async def cmd_stats(u,c):
    ...
    for coin in MARKETS:
        ...
        msg+=(em+" <b>"+coin+"</b>\n"
            "Trades: <b>"+str(t)+"</b> (L:"+str(lt)+" S:"+str(sh)+")\n"
            "WR: <b>"+str(wr)+"%</b> | "+icon+" <b>"+sg+str(g)+"%</b>\n"
            "PnL: <b>$"+str(round(s["total_pnl"],4))+"</b>\n"
            "\U0001f4b0 Margin: <b>$"+str(s["current_margin"])+"</b>\n"
            "\U0001f3c6 Peak: <b>$"+str(s["peak_margin"])+"</b>\n\n")
    await u.message.reply_text(msg,parse_mode="HTML")
    ...
    for coin in MARKETS:
        ...
        if not h:
            ...
            for t in reversed(h[-5:]):
                ...
                msg+=icon+" #"+str(t["no"])+" "+t["side"]+" ["+t["reason"]+"] <b>"+sg+"$"+str(t["pnl"])+"</b> ("+pct_sg+str(t.get("pct",0))+"%) $"+str(t["new_margin"])+"\n"
            msg+="\n"
    await u.message.reply_text(msg,parse_mode="HTML")
...
async def cmd_balance(u,c):
    try:
        ...
        msg+="\U0001f4a0 Equity: <b>$"+str(equity)+"</b>\n"
        msg+="\u2705 Available: <b>$"+str(avail)+"</b>\n\n"
        for coin in MARKETS:
            s=state[coin]["stats"]; em=MARKETS[coin]["emoji"]
            msg+=em+" <b>"+coin+"</b> Margin: <b>$"+str(s["current_margin"])+"</b>\n"
        await u.message.reply_text(msg,parse_mode="HTML")
    except Exception as e:
...
async def main():
    ...
    await send_tg(
        ...
        +MARKETS["ETH"]["emoji"]+" ETH <b>$"+str(state["ETH"]["stats"]["current_margin"])+"</b>"
        +" | "+MARKETS["BNB"]["emoji"]+" BNB <b>$"+str(state["BNB"]["stats"]["current_margin"])+"</b>\n"
