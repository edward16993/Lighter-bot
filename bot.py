...
async def cmd_balance(u,c):
    try:
        ...
        upnl_str=str(round(float(upnl),2)) if upnl is not None else "N/A"
        lines=["Balance"]
        ...
        lines.append("")
        lines.append("Keys: "+str(acct_keys))
...
async def daily_report():
    ...
    while True:
        ...
        try:
            lines=["*Daily Report - "+datetime.now().strftime("%d %b")+"*"]
            lines.append("---")
            total_pnl=0.0
            for coin in MARKETS:
                em=MARKETS[coin]["emoji"]
            ...
            lines.append("Total PnL: *"+sg+"$"+str(round(total_pnl,4))+"*")
            await send_tg("\n".join(lines))
        except Exception as e:
