...
async def cmd_balance(u,c):
    try:
        ...
        for k,v in r_dict.items():
            if "collateral" in k.lower() or "balance" in k.lower() or "equity" in k.lower():
                if col is None and v is not None: col=v
            if "pnl" in k.lower() or "unrealized" in k.lower():
                if upnl is None and v is not None: upnl=v
        col_str=str(round(float(col),2)) if col is not None else "?"
        upnl_str=str(round(float(upnl),2)) if upnl is not None else "?"
        keys=list(r_dict.keys())[:8]
        msg="*Balance*\nCollateral: $*"+col_str+"*\nUnrealized: $*"+upnl_str+"*\n\n"
        for coin in MARKETS:
            s=state[coin]["stats"]; em=MARKETS[coin]["emoji"]
            msg+=em+" *"+coin+"* Margin: $*"+str(s["current_margin"])+"*\n"
        msg+="\n_API keys: "+str(keys)+"_"
        await u.message.reply_text(msg,parse_mode="Markdown")
    except Exception as e:
